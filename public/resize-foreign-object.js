(function (win) {
  ...
  win.fixedSizeForeignObject = function fixedSizeForeignObject(el) {
    if (!svgs) {
      svgs = [];
      // win.addEventListener("resize", resizeSVGs, false);
    }
    ...
    let info = {
      el: el,
      svg: svg,
      w: el.getAttribute("width") * 1,
      h: el.getAttribute("height") * 1,
      x: el.getAttribute("x") * 1,
      y: el.getAttribute("y") * 1,
    };
    els.push(info);
  ...
  function fixScale(info) {
    ...
    info.el.setAttribute("data-resized", true);
